...
public class NpcExitStationAction : NpcAction
{
    #region 常量字段
    //导航到目的地位置距离差
    ...
    #region 状态字段
    //Npc行为流程
    private PointStatus[] ExitStationTrainUpAction_StepArray = new PointStatus[]
    {
        PointStatus.Train_Down,
        PointStatus.NpcDownTrain_Down,
    ...
    protected override void Awake()
    {
        ...
        m_endStepIndex = ExitStationTrainUpAction_StepArray.Length - 1;
    }
    void Start()
    {
        StartAction();
    ...
    {
        int pointStatus = (int)ExitStationTrainUpAction_StepArray[m_startStepIndex];
        Point point = StationEngine.Instance.GetFirstPoint2RandomPointQueue(m_stationIndex, pointStatus);
        return point;
    }
    //获得一个随机的出站位置点
    private Point GetRandomExitStationPoint()
    {
        int pointStatus = (int)ExitStationTrainUpAction_StepArray[m_endStepIndex];
        Point point = StationEngine.Instance.GetFirstPoint2RandomPointQueue(m_stationIndex, pointStatus);
        return point;
    }
    //获得一个当前步骤的没有预约的位置点
    public Point GetNoReservationPoint2RandomPointQueue()
    {
        Point point = StationEngine.Instance.GetNoReservationPoint2RandomPointQueue(m_stationIndex,
            (int)ExitStationTrainUpAction_StepArray[m_startStepIndex]);
        return point;
    }
    //获得一个当前步骤对应的休息区的位置点
    ...
    {
        Point point = null;
        int restStatus = (int)ExitStationTrainUpAction_StepArray[m_startStepIndex] + 1;
        //排队买票休息区
        //排队进站检票休息区
        //排队上车休息区
    ...
    {
        Point point = null;
        PointStatus status = ExitStationTrainUpAction_StepArray[stepIndex];
            ...
            status == PointStatus.Train_Up) && m_gotoPoint != null)
        {
            ...
            point = StationEngine.Instance.GetFirstPoint(m_stationIndex,
                (int)ExitStationTrainUpAction_StepArray[stepIndex],
    ...
    private void StartAction()
    {
        ...
        if (PointStatus.EnterStation == ExitStationTrainUpAction_StepArray[m_startStepIndex])
        {
            //这个gotoPoint不可能为null
            ...
            if (m_gotoPoint != null)
            {
                ...
                if (m_gotoPoint.IsRestAreaPoint)
                {
                    ...
                    if (tempPoint != null)
                    {
                        ...
                        tempPoint = StationEngine.Instance.GetReverseNoReservationPointByQueueIndex(m_stationIndex,
                            (int)ExitStationTrainUpAction_StepArray[m_startStepIndex], queueIndex);
                        if (tempPoint != null)
                        {
                            GotoDestination(tempPoint);
                        }
                ...
                if (Vector2.Distance(m_selfPosV2, m_desPosV2) < m_navDistance)
                {
                    ...
                    else if (m_gotoPoint != null && m_gotoPoint.m_nextPoint == null && m_gotoPoint.IsRestAreaPoint == false)
                    {
                        ...
                        if (m_gotoPoint.m_pointStatus == ExitStationTrainUpAction_StepArray[m_endStepIndex])
                        {
                            //销毁NPC
                            DestroyNpc();
                            yield break;
