...
public class NpcEnterStationDownAction : NpcAction {
    #region 常量字段
    //导航到目的地位置距离差
    ...
    protected override void StartAction () {
        ...
        } else if (PointStatus.WaitTrain_Down == m_stepArray[m_startStepIndex]) {
        ...
        while (!IsDestroy) {
            ...
            if (m_gotoPoint != null) {
                ...
                if (m_gotoPoint.IsRestAreaPoint) {
                    if (PointStatus.WaitTrain_Down == m_stepArray[m_startStepIndex]) {
                        //站台排队0-32个队列位置中选择，因为车只有16个门
                        tempPoint = GetNoReservationPoint2RandomPointQueue (0, 32);
                    } else {
                        tempPoint = GetNoReservationPoint2RandomPointQueue ();
