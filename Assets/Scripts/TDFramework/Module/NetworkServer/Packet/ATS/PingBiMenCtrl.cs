...
public class PingBiMenCtrl : IPacket
{
    #region 字段和属性
    //屏蔽门所对应的站台索引
    ...
    //屏蔽门开关状态
    public byte m_statusFlag;         //设备行为状态值 0表示关，1表示打开 0-255的int都可以默认正确转成byte
    //数据占用字节大小
    public UInt16 Size
    {
        get { return 4; }
    ...
    #region 构造函数
    public PingBiMenCtrl()
    {

    }
    public PingBiMenCtrl(byte[] bytes)
    {
        if (bytes.Length <= 0)
        {
            return;
